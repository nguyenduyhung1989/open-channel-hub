...
Phases 4h–4j are candidate-only changes. They have not yet received final
...
Bot-identity evidence; Phase 4j adds only immutable dashboard-principal
Telegram authorization evidence. Neither adds a sender.
...
| Candidate Phase 4j code permits a separately configured dashboard principal to record one immutable Telegram delivery-authorization fact only for a currently eligible queued command. It binds the row to inbox/principal/scope/Bot fingerprints and excludes private target, text, bearer/session, credentials, provider response, attempt, receipt, retry, and mutable state.                                                                                                                                                                                                            | Verification is pending. The candidate renders only a non-secret command reference to an authenticated authorized principal and repeats durable checks in PostgreSQL. Its final disposable Compose proof is designed to use a synthetic dashboard configuration and manually forwarded signed cookie to exercise that writer; it remains no browser/TLS or provider-request claim.                                                                                          | Current send policy, multi-principal separation of duties, dispatcher/lease, timeout uncertainty, receipt mapping, retry policy, live-provider/TLS, and production verification. |
...
## Phase 4j candidate Telegram delivery-authorization boundary
