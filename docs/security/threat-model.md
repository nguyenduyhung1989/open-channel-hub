...
**Status:** Phase 0 has GitHub CI/CodeQL evidence at <code>8b80c3b</code>, the
Phase 1a candidate at <code>7141949</code>, and the Phase 2a storage candidate
at <code>f106bb8</code>. The uncommitted Phase 2b candidate adds an
operator-authenticated canonical event-read path and an expanded synthetic
Compose smoke check. Its exact local and GitHub verification remain open.

## Facts before plans

| Present in the Phase 2b source                                                                                                                                               | Verified local synthetic evidence                                                                                                                                                                                                                                                                                     | Absent or planned only                                                                                                                       |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| A small HTTP API with liveness and readiness routes; typed startup configuration; source-offer route/header; official Telegram Bot boundary; operator event-read route.      | A fake migration ran twice, a fake duplicate webhook returned <code>204</code> twice, and one ledger row remained. An unauthenticated HTTPS reachability probe to <code>api.telegram.org</code> returned <code>200</code>; no real token, Bot API method request, webhook registration, or customer content was used. | Fresh GitHub checks for the Phase 2b candidate, a real Telegram/TLS flow, and a production deployment.                                       |
...
| Canonical inbound persistence and read paths use parameterized SQL; raw provider payloads are excluded. The read route is token-gated, connection-fixed, and cursor-bounded. | The duplicate synthetic provider event was stored only once.                                                                                                                                                                                                                                                          | A user inbox, attachments, user/organization authorization, Redis, queue/outbox, retry, and audit trail.                                     |
...
| SQL injection or query abuse                              | Storage uses fixed schema-qualified SQL and positional parameters. The read route bounds page size, rejects malformed opaque cursors before storage access, and never accepts a caller-selected connection ID. Raw provider payloads are not stored. | Adapter integration coverage against production-sized data and ongoing review of every new query.                                                 |
...
| Operator event-read disclosure or traversal inconsistency | Bearer authentication precedes query parsing; reads are fixed to the configured connection. Keyset cursors retain one sequence snapshot, and appends serialize sequence allocation before commit.                                                    | Token rotation, user/RBAC model, audit trail, rate limiting, access logging policy, and production load testing.                                  |
