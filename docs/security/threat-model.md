...
after final local checks, an independent audit, a synthetic Compose/PostgreSQL
...
| Runtime multi-connection JSON is parsed only from an absolute secret file. It accepts strict `telegram_bot`, `zalo_oa`, `facebook_page`, and `whatsapp_business` entries, optional `inboxes`, and optional `dashboard` principals with an exact public HTTPS origin and exact-profile Argon2id password hashes. The sole local exception is exact `http://localhost:<port>` for a loopback Docker demonstration; IPs and every other HTTP host remain invalid. Phase 4f adds optional principal `replyIntentInboxIds` as an explicit subset of readable inboxes.                            | Configuration loader tests use synthetic documents and reject malformed/duplicate/unsafe input without leaking content. A Meta App used by both Facebook Page and WhatsApp requires matching credentials and one common declared `/v1/webhooks/meta` callback. Phase 4f checks passed at `74fca30`.                                                                                                                                                                         | Managed secret store, rotation procedure, host hardening, and audit logging.                                                                                                     |
...
| Verified Phase 4h source creates one immutable authorization-provenance row alongside each new reply command. It stores only kind, configured inbox ID, optional dashboard principal ID, scope fingerprint, and time; old command rows are not backfilled.                                                                                                                                                                                                                                                                                                                                  | Final local checks, independent audit, synthetic Compose/PostgreSQL proof, GitHub CI, and CodeQL passed at `52608e0`. The source has no public API field or browser field for provenance and no provider I/O.                                                                                                                                                                                                                                                               | A current authorization recheck, legacy-command policy, provider eligibility, dispatcher, retry policy, and production deployment.                                               |
| Verified Phase 4i source preserves the allowed Telegram chat type only internally, binds new Telegram registrations to an opaque Bot fingerprint, and records an immutable private-chat eligibility row with a new Telegram command. Historic rows are not backfilled or adopted.                                                                                                                                                                                                                                                                                                           | Final local checks, independent audit, synthetic Compose/PostgreSQL proof, GitHub CI, and CodeQL passed at `52608e0`. The source does not project chat type or fingerprint to public readers/dashboard and makes no provider request.                                                                                                                                                                                                                                       | Current provider policy, command selection, sender, attempt ordering, timeout/receipt handling, live-provider/TLS, and production verification.                                  |
| Verified Phase 4j source permits a separately configured dashboard principal to record one immutable Telegram delivery-authorization fact only for a currently eligible queued command. It binds the row to inbox/principal/scope/Bot fingerprints and excludes private target, text, bearer/session, credentials, provider response, attempt, receipt, retry, and mutable state.                                                                                                                                                                                                           | Final local checks, independent audit, synthetic Compose/PostgreSQL proof, GitHub CI, and CodeQL passed at `52608e0`. The source renders only a non-secret command reference to an authenticated authorized principal and repeats durable checks in PostgreSQL; its disposable Compose proof remains no browser/TLS or provider-request claim.                                                                                                                              | Current send policy, multi-principal separation of duties, dispatcher/lease, timeout uncertainty, receipt mapping, retry policy, live-provider/TLS, and production verification. |
...
## Phase 4h verified authorization-provenance boundary
...
## Phase 4i verified Telegram private-reply boundary
...
## Phase 4j verified Telegram delivery-authorization boundary

- `0013_outbound_telegram_delivery_authorizations` has at most one immutable
  row per Phase 4i eligibility record. It stores only command ID, configured
  inbox/principal, SHA-256 scope fingerprint, opaque Bot fingerprint, and
  PostgreSQL time. Its primary/foreign key and update/delete-rejection trigger
  prevent a second or changed authorization fact.
- Optional `dashboard.principals[].telegramDeliveryAuthorizationInboxIds` is a
  strict subset of readable inboxes and defaults to an empty immutable set.
  The capability is separate from both inbox bearers and Phase 4f intent
  recording, so a reader cannot self-grant durable approval through a URL or
  browser field.
- `GET /operator/outbound-commands` renders an authorization form only for a
  server-computed eligible row. `POST /operator/telegram-delivery-authorizations`
  requires exact HTTPS origin, signed session, anti-forgery value, and the
  configured approval scope before it invokes the writer. The writer repeats
  Phase 4h provenance, Phase 4i private/Bot, current registry, no-attempt, and
  no-prior-authorization checks in its transaction.
- Private targets, `conversationId`, `senderId`, source message IDs, text
  inputs, bearer/provider credentials, scope/Bot fingerprints, attempt
  controls, provider requests, retry controls, and delivery claims do not
  enter the form or its result. The one-operator alpha may self-authorize; this
  is explicitly not four-eyes policy.
...
pepper, webhook, and optional file-backed Google OAuth client values. It is
trusted only to the extent that the operator
...
The verified Phase 4h source keeps the authority tuple server-side as well: the
...
The optional Phase 4k Google boundary keeps its client ID/secret in separate
API-only secret files, keeps PKCE transaction material in memory, and sends
only a signed opaque transaction ID to the browser. It accepts a Google subject
only after server-side code exchange and ID-token nonce/audience verification.
...
The optional Phase 4k identity table contains only a domain-separated HMAC of
a verified Google `sub`, one configured principal ID, and binding time. It is
immutable and one-to-one. It excludes raw Google account/profile data, ID,
access, and refresh tokens, browser session values, inbox bearers, and provider
credentials.

The verified Phase 4h source adds a separate append-only provenance table. It retains
...
The verified Phase 4i source adds an internal inbound chat-type field and a separate
...
The verified Phase 4h source records provenance inside that final storage
...
For a verified Phase 4i Telegram command, the same transaction additionally
...
- Dashboard session integrity, anti-forgery values, exact configured origin,
...
| Dashboard session theft, CSRF, Google callback abuse, or scope escalation              | The optional dashboard is server-rendered without a browser bearer or client-side inbox API. It authenticates configured principals with exact-profile Argon2id hashes or an optional configured Google identity. Browser session and temporary OAuth transaction cookies are signed `__Host-` `Secure` `HttpOnly` `SameSite=Lax`; Lax is limited to the top-level Google callback, while every write retains exact-origin checks and hidden anti-forgery values. The OAuth flow uses server-side authorization-code PKCE, state, nonce, audience-verified ID tokens, an explicit verified-email check, one-time in-memory transactions, and HMAC-only immutable Google-subject links. One optional deployment-local bootstrap email can bind only one existing configured principal; it creates no role or inbox scope and its raw value is never written to PostgreSQL. Unknown Google accounts cannot provision or claim a principal. The server resolves only the principal's preconfigured inboxes; URL input cannot add a connection. | Public TLS/proxy proof, cross-instance edge rate limiting, cookie/header log-redaction verification, password/secret rotation drill, Google-console redirect/client administration review, audit trail, and a full authorization design. |
| Unregistered or drifting connection identity                                           | Startup registers manifest-derived immutable metadata; PostgreSQL blocks new event rows without a registry row; changed metadata for an existing ID fails registration. Zalo OA, Facebook Page, WhatsApp Business, and verified Telegram registrations require domain-separated SHA-256 fingerprints of configured provider identities. The first identity binding is refused when pre-registry history already uses that ID; Telegram's fingerprint derives only from the configured token's numeric Bot-ID prefix.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Historical-row reconciliation, a deliberate later foreign-key validation migration, and a provider-specific live identity policy before dispatch.                                                                                        |
...
| Container privilege or network exposure                                                | API/migration are non-root, drop capabilities, use `no-new-privileges`, and use an internal data network. API host access is loopback-only. Normal local Compose configuration omits the dashboard. Exact `http://localhost:<port>` is permitted only for a loopback Docker demonstration; production dashboard sessions still require an external HTTPS origin. The Phase 4j disposable smoke configuration enables it only to manually return a signed `Secure` cookie to `curl` for server/PostgreSQL coverage; this does not prove browser HTTPS behavior.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | TLS proxy, resource limits, monitoring, host hardening, and a secret-delivery design that permits read-only roots.                                                                                                                       |
...
- `NOT VALID` means PostgreSQL enforces the new foreign key for new rows but
  ...
  parent.
- Zalo OA, Facebook Page, WhatsApp Business, and verified Telegram
  fingerprints protect configured provider-identity reuse after history exists
  without retaining raw account IDs. Telegram's verified binding comes only
...
- The verified Phase 4h source's authority kind, configured inbox ID, optional
...
- The verified Phase 4i source's `private` eligibility record is a historical
