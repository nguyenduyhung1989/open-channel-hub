...
| Dashboard session theft, CSRF, Google callback abuse, or scope escalation              | The optional dashboard is server-rendered without a browser bearer or client-side inbox API. It authenticates configured principals with exact-profile Argon2id hashes or an optional pre-linked Google identity. Browser session and temporary OAuth transaction cookies are signed `__Host-` `Secure` `HttpOnly` `SameSite=Lax`; Lax is limited to the top-level Google callback, while every write retains exact-origin checks and hidden anti-forgery values. The OAuth flow uses server-side authorization-code PKCE, state, nonce, audience-verified ID tokens, one-time in-memory transactions, and HMAC-only immutable Google-subject links. Unknown Google accounts cannot provision or claim a principal. The server resolves only the principal's preconfigured inboxes; URL input cannot add a connection.                                                                                                        | Public TLS/proxy proof, cross-instance edge rate limiting, cookie/header log-redaction verification, password/secret rotation drill, Google-console redirect/client administration review, audit trail, and a full authorization design. |
