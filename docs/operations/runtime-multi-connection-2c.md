...
server-rendered dashboard described below, which remains read-only. The Phase
4e candidate reuses that dashboard configuration for
...
[Phase 4e candidate guide](operator-dashboard-queued-history-4e.md) for the
...
hashing, proxy controls, session rotation, and limits. The Phase 4e candidate
...
| Read dashboard queued history (Phase 4e candidate) | <code>GET /operator/outbound-commands</code>                                       | The signed dashboard session selects only a preconfigured inbox server side. The browser receives no inbox bearer; the fixed 50-row continuation reuses the Phase 4d inbox/scope-bound history cursor.                                                                  |
